...
public class ZipCompress {
	...
	public void zip(String zipFileName, String sourceFileName) throws Exception {
		...
		File[] sourceFile = (new File(sourceFileName)).listFiles();
		//调用函数
		for (int i = 0; i < sourceFile.length; i++) {
			compress(out, bos, sourceFile[i], sourceFile[i].getName());
		}
		bos.close();
		out.close();
	...
	public static void main(String[] args) {
		...
		try {
			zipCom.zip("D:\\html.zip", "D:\\workspace\\daya_dsa\\similarity-analysis_local_test\\target\\similarity_analysis_local_tes\\pages\\html");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
